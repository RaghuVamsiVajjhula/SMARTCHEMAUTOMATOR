...
# -------------------------
# Helper: robust selector wait (avoids API mismatch on wait_for_selector kw)
# -------------------------
def wait_for_selector_safe(page, selector, timeout_ms=5000, poll_interval=0.25):
    """
    Polls page.query_selector(selector) until found or timeout_ms elapsed.
    Returns True if found, False otherwise.
    This avoids differences/bugs across Playwright versions for wait_for_selector kw.
    """
    elapsed = 0.0
...
def login(page, pause_after=False):
    ...
    if not wait_for_selector_safe(page, SELECTOR_USERNAME, timeout_ms=8000):
...
def search_chemical(page, chemical):
    ...
    # try direct fill, then click+type, then JS fallback
    filled = False
    try:
    ...
    # submit: try clicking anchor, else JS submit
    try:
    ...
    # JS fallback (call submitChemicalSearchForm if available)
    try:
...
def click_applications_tab(page):
    # try a few ways: visible text, href pattern for details function, or li containing Applications
    candidates = [
        'text=Applications',
...
# -------------------------
# Helper: extract URL from onclick string
# -------------------------
...
# -------------------------
# Download Applications Excel (robust)
# -------------------------
def download_applications_excel(page, save_folder="downloads"):
    ...
    # wait for either anchor or image to appear
    found_sel = None
    ...
    # Attempt 1: click + expect_download in current page context
    try:
    ...
    try:
        ...
        # open new page and capture download
        ctx = page.context
